...
class Chatbot {
    ...
    // Initialize event listeners for chatbot UI
    initializeChatbot() {
    ...
    async sendMessage() {
        if (!this.chatInput || !this.messagesDiv) return;
        ...
        try {
            // Call backend
            const response = await fetch("https://my-bot-personal.onrender.com/chat", {
                method: "POST",
            ...
            // Remove typing and append bot message
            this.removeTyping(typingEl);
            this.appendMessage(data.reply, 'bot-message');

        } catch (err) {
            this.removeTyping(typingEl);
            this.appendMessage("Sorry, I couldn't reach the server.", 'bot-message');
            console.error(err);
        }
    ...
    removeTyping(el) {
        if (el && this.messagesDiv.contains(el)) {
            this.messagesDiv.removeChild(el);
        }
    }
